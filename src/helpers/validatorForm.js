...
export const validatorLogin = form => {
  ...
  if (form.password.includes(' ')) {
    ...
    errorsLogin.password =
      'Debe contener un dígito numerico, una letra mayúscula y una minuscula';
  }

  return errorsLogin;
};
export const validatorRegister = form => {
  let errorsRegister = {};

  const regexName = /^[A-Za-zÑñÁáÉéÍíÓóÚúÜü\s]{3,15}$/;

  // Suponiendo que validatorLogin devuelve un objeto de errores
  const errorsLogin = validatorLogin(form);

  // Combinar los errores de ambas funciones
  errorsRegister = { ...errorsRegister, ...errorsLogin };

  if (form.name.includes(' ')) {
    errorsRegister.name = 'No puede contener espacios vacíos';
  } else if (!regexName.test(form.name.trim())) {
    errorsRegister.name =
      'El campo solo acepta letras y un rango de 3 a 15 caracteres';
  }
